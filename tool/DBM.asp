...
.bozo{
width:150px;line-height:25px;height:25px;border:1px solid #B5BBBF; background-color: #fff;margin:5px 5px;
text-align: center;
display: block;
;}
.bozo1{
...
if session("dbtype") = "" or session("dbstr") = "" then
	...
	<form action="?key=db" method="post" name="dbt">
		  ...
		  <input type="submit" name="Submit" value="连接" class="bozo" /><br><br>
	...
	<form name="createdatabase" method="post" action="?key=createdatabase">
	  ...
	  <input type="submit" name="Submit" value="创建" class="bozo">
	</form>
	
	<p style="padding-top:20px;">其他数据库管理：
	<a href='/bozo/tool/adminer.php' target="_blank"><div class="bozo1">Adminer管理</div></a><a href='http://sx.bozoyan.com' target="_blank"><div class="bozo1">PhpMyAdmin管理</div></a></p>
	<%
	response.End()
end if
