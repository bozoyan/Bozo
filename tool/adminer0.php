...
confirm(){return" onclick=\"return confirm('".'ä½ ç¡®å®šå—ï¼Ÿ'."');\"";}function
...
sprintf('%.3f ç§’',max(0,$ic-$wg));}function
...
upload_error($l){$be=($l==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($l?'ä¸èƒ½ä¸Šä¼ æ–‡ä»¶ã€‚'.($be?" ".sprintf('æœ€å¤šå…è®¸çš„æ–‡ä»¶å¤§å°ä¸º %sB',$be):""):'æ–‡ä»¶ä¸å­˜åœ¨ã€‚');}function
...
h($g->error);}function
check_sqlite_name($C){global$g;$_c="db|sdb|sqlite";if(!preg_match("~^[^\\0]*\\.($_c)\$~",$C)){$g->error=sprintf('è¯·ä½¿ç”¨è¿™äº›æ‰©å±•ä¸­çš„ä¸€ä¸ªï¼š%sã€‚',str_replace("|",", ",$_c));return
false;}return
true;}function
create_database($k,$d){global$g;if(file_exists($k)){$g->error='æ–‡ä»¶å·²å­˜åœ¨ã€‚';return
...
as$k){if(!@unlink($k)){$g->error='æ–‡ä»¶å·²å­˜åœ¨ã€‚';return
false;}}return
true;}function
rename_database($C,$d){global$g;if(!check_sqlite_name($C))return
false;$g->Min_SQLite(":memory:");$g->error='æ–‡ä»¶å·²å­˜åœ¨ã€‚';return@rename(DB,$C);}function
...
set_schema($dg){global$g,$rh,$_g;$J=$g->query("SET search_path TO ".idf_escape($dg));foreach(types()as$U){if(!isset($rh[$U])){$rh[$U]=0;$_g['ç”¨æˆ·ç±»å‹'][]=$U;}}return$J;}function
...
unconvert_field($m,$J){return$J;}function
support($Cc){return
preg_match('~^(columns|database|drop_col|indexes|processlist|scheme|sql|status|table|variables|view|view_trigger)$~',$Cc);}$w="oracle";$rh=array();$_g=array();foreach(array('æ•°å­—'=>array("number"=>38,"binary_float"=>12,"binary_double"=>21),'æ—¥æœŸæ—¶é—´'=>array("date"=>10,"timestamp"=>29,"interval year"=>12,"interval day"=>28),'å­—ç¬¦ä¸²'=>array("char"=>2000,"varchar2"=>4000,"nchar"=>2000,"nvarchar2"=>4000,"clob"=>4294967295,"nclob"=>4294967295),'äºŒè¿›åˆ¶'=>array("raw"=>2000,"long raw"=>2147483648,"blob"=>4294967295,"bfile"=>4294967296),)as$x=>$X){$rh+=$X;$_g[$x]=array_keys($X);}$yh=array();$He=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT REGEXP","NOT IN","IS NOT NULL","SQL");$Sc=array("length","lower","round","upper");$Wc=array("avg","count","count distinct","max","min","sum");$bc=array(array("date"=>"current_date","timestamp"=>"current_timestamp",),array("number|float|double"=>"+/-","date|timestamp"=>"+ interval/- interval","char|clob"=>"||",));}$Tb["mssql"]="MS SQL";if(isset($_GET["mssql"])){$pf=array("SQLSRV","MSSQL");define("DRIVER","mssql");if(extension_loaded("sqlsrv")){class
...
unconvert_field($m,$J){return$J;}function
support($Cc){return
preg_match('~^(columns|database|drop_col|indexes|scheme|sql|table|trigger|view|view_trigger)$~',$Cc);}$w="mssql";$rh=array();$_g=array();foreach(array('æ•°å­—'=>array("tinyint"=>3,"smallint"=>5,"int"=>10,"bigint"=>20,"bit"=>1,"decimal"=>0,"real"=>12,"float"=>53,"smallmoney"=>10,"money"=>20),'æ—¥æœŸæ—¶é—´'=>array("date"=>10,"smalldatetime"=>19,"datetime"=>19,"datetime2"=>19,"time"=>8,"datetimeoffset"=>10),'å­—ç¬¦ä¸²'=>array("char"=>8000,"varchar"=>8000,"text"=>2147483647,"nchar"=>4000,"nvarchar"=>4000,"ntext"=>1073741823),'äºŒè¿›åˆ¶'=>array("binary"=>8000,"varbinary"=>8000,"image"=>2147483647),)as$x=>$X){$rh+=$X;$_g[$x]=array_keys($X);}$yh=array();$He=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT IN","IS NOT NULL");$Sc=array("len","lower","round","upper");$Wc=array("avg","count","count distinct","max","min","sum");$bc=array(array("date|time"=>"getdate",),array("int|decimal|real|float|money|datetime"=>"+/-","char|text"=>"+",));}$Tb["simpledb"]="SimpleDB";if(isset($_GET["simpledb"])){$pf=array("SimpleXML");define("DRIVER","simpledb");if(class_exists('SimpleXMLElement')){class
...
<tr><th>ç³»ç»Ÿ<td>',html_select("auth[driver]",$Tb,DRIVER,"loginDriver(this);"),'<tr><th>æœåŠ¡å™¨<td><input name="auth[server]" value="',h(SERVER),'" title="hostname[:port]" placeholder="localhost" autocapitalize="off">
<tr><th>ç”¨æˆ·å<td><input name="auth[username]" id="username" value="',h($_GET["username"]),'" autocapitalize="off">
<tr><th>å¯†ç <td><input type="password" name="auth[password]">
<tr><th>æ•°æ®åº“<td><input name="auth[db]" value="',h($_GET["db"]);?>" autocapitalize="off">
</table>
...
echo"<p><input type='submit' value='".'ç™»å½•'."'>\n",checkbox("auth[permanent]",1,$_COOKIE["adminer_permanent"],'ä¿æŒç™»å½•')."\n";}function
login($Sd,$G){return
true;}function
tableName($Gg){return
h($Gg["Name"]);}function
fieldName($m,$Me=0){return'<span title="'.h($m["full_type"]).'">'.h($m["field"]).'</span>';}function
selectLinks($Gg,$O=""){echo'<p class="links">';$Rd=array("select"=>'é€‰æ‹©æ•°æ®');if(support("table")){$Rd["table"]='æ˜¾ç¤ºç»“æ„';if(is_view($Gg))$Rd["view"]='ä¿®æ”¹è§†å›¾';else$Rd["create"]='ä¿®æ”¹è¡¨';}if($O!==null)$Rd["edit"]='æ–°å»ºæ•°æ®';foreach($Rd
...
selectQuery($H){global$w;return($_GET["edit"]!=""?"":"<p><code class='jush-$w'>".h(str_replace("\n"," ",$H))."</code>".(support("sql")?" <a href='".h(ME)."sql=".urlencode($H)."'>".'ç¼–è¾‘'."</a>":"")."</p>");}function
rowDescription($Q){return"";}function
rowDescriptions($L,$Kc){return$L;}function
selectLink($X,$m){}function
selectVal($X,$_,$m){$J=($X===null?"<i>NULL</i>":(preg_match("~char|binary~",$m["type"])&&!preg_match("~var~",$m["type"])?"<code>$X</code>":$X));if(preg_match('~blob|bytea|raw|file~',$m["type"])&&!is_utf8($X))$J=sprintf('%d å­—èŠ‚',strlen(html_entity_decode($X,ENT_QUOTES)));return($_?"<a href='".h($_)."'>$J</a>":$J);}function
editVal($X,$m){return$X;}function
selectColumnsPrint($M,$f){global$Sc,$Wc;print_fieldset("select",'é€‰æ‹©',$M);$r=0;$M[""]=array();foreach($M
...
selectSearchPrint($Z,$f,$v){print_fieldset("search",'æœç´¢',$Z);foreach($v
as$r=>$u){if($u["type"]=="FULLTEXT"){echo"(<i>".implode("</i>, <i>",array_map('h',$u["columns"]))."</i>) AGAINST"," <input type='search' name='fulltext[$r]' value='".h($_GET["fulltext"][$r])."' onchange='selectFieldChange(this.form);'>",checkbox("boolean[$r]",1,isset($_GET["boolean"][$r]),"BOOL"),"<br>\n";}}$_GET["where"]=(array)$_GET["where"];reset($_GET["where"]);$Xa="this.nextSibling.onchange();";for($r=0;$r<=count($_GET["where"]);$r++){list(,$X)=each($_GET["where"]);if(!$X||("$X[col]$X[val]"!=""&&in_array($X["op"],$this->operators))){echo"<div>".select_input(" name='where[$r][col]' onchange='$Xa'",$f,$X["col"],"(".'ä»»æ„ä½ç½®'.")"),html_select("where[$r][op]",$this->operators,$X["op"],$Xa),"<input type='search' name='where[$r][val]' value='".h($X["val"])."' onchange='".($X?"selectFieldChange(this.form)":"selectAddRow(this)").";' onkeydown='selectSearchKeydown(this, event);' onsearch='selectSearchSearch(this);'></div>\n";}}echo"</div></fieldset>\n";}function
selectOrderPrint($Me,$f,$v){print_fieldset("sort",'æ’åº',$Me);$r=0;foreach((array)$_GET["order"]as$x=>$X){if($X!=""){echo"<div>".select_input(" name='order[$r]' onchange='selectFieldChange(this.form);'",$f,$X),checkbox("desc[$r]",1,isset($_GET["desc"][$x]),'é™åº')."</div>\n";$r++;}}echo"<div>".select_input(" name='order[$r]' onchange='selectAddRow(this);'",$f),checkbox("desc[$r]",1,false,'é™åº')."</div>\n","</div></fieldset>\n";}function
selectLimitPrint($z){echo"<fieldset><legend>".'èŒƒå›´'."</legend><div>";echo"<input type='number' name='limit' class='size' value='".h($z)."' onchange='selectFieldChange(this.form);'>","</div></fieldset>\n";}function
selectLengthPrint($Vg){if($Vg!==null){echo"<fieldset><legend>".'æ–‡æœ¬æ˜¾ç¤ºé™åˆ¶'."</legend><div>","<input type='number' name='text_length' class='size' value='".h($Vg)."'>","</div></fieldset>\n";}}function
selectActionPrint($v){echo"<fieldset><legend>".'åŠ¨ä½œ'."</legend><div>","<input type='submit' value='".'é€‰æ‹©'."'>"," <span id='noindex' title='".'Full table scan'."'></span>","<script type='text/javascript'>\n","var indexColumns = ";$f=array();foreach($v
...
messageQuery($H){global$w;restart_session();$ad=&get_session("queries");$s="sql-".count($ad[$_GET["db"]]);if(strlen($H)>1e6)$H=preg_replace('~[\x80-\xFF]+$~','',substr($H,0,1e6))."\n...";$ad[$_GET["db"]][]=array($H,time());return" <span class='time'>".@date("H:i:s")."</span> <a href='#$s' onclick=\"return !toggle('$s');\">".'SQLå‘½ä»¤'."</a>"."<div id='$s' class='hidden'><pre><code class='jush-$w'>".shorten_utf8($H,1000).'</code></pre>'.(support("sql")?'<p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($ad[$_GET["db"]])-1)).'">'.'ç¼–è¾‘'.'</a>':'').'</div>';}function
editFunctions($m){global$bc;$J=($m["null"]?"NULL/":"");foreach($bc
as$x=>$Sc){if(!$x||(!isset($_GET["call"])&&(isset($_GET["select"])||where($_GET)))){foreach($Sc
as$hf=>$X){if(!$hf||preg_match("~$hf~",$m["type"]))$J.="/$X";}if($x&&!preg_match('~set|blob|bytea|raw|file~',$m["type"]))$J.="/SQL";}}if($m["auto_increment"]&&!isset($_GET["select"])&&!where($_GET))$J='è‡ªåŠ¨å¢é‡';return
explode("/",$J);}function
editInput($Q,$m,$Ha,$Y){if($m["type"]=="enum")return(isset($_GET["select"])?"<label><input type='radio'$Ha value='-1' checked><i>".'åŸå§‹'."</i></label> ":"").($m["null"]?"<label><input type='radio'$Ha value=''".($Y!==null||isset($_GET["select"])?"":" checked")."><i>NULL</i></label> ":"").enum_input("radio",$Ha,$m,$Y,0);return"";}function
...
as$N=>$Eh){foreach($Eh
as$V=>$G){if($G!==null){if($Hc){echo"<p id='logins' onmouseover='menuOver(this, event);' onmouseout='menuOut(this);'>\n";$Hc=false;}$Fb=$_SESSION["db"][$Jh][$N][$V];foreach(($Fb?array_keys($Fb):array(""))as$k)echo"<a href='".h(auth_url($Jh,$N,$V,$k))."'>($Tb[$Jh]) ".h($V.($N!=""?"@$N":"").($k!=""?" - $k":""))."</a><br>\n";}}}}}else{$this->databasesPrint($me);if(DB==""||!$me){echo"<p class='links'>".(support("sql")?"<a href='".h(ME)."sql='".bold(isset($_GET["sql"])&&!isset($_GET["import"])).">".'SQLå‘½ä»¤'."</a>\n<a href='".h(ME)."import='".bold(isset($_GET["import"])).">".'å¯¼å…¥'."</a>\n":"")."";if(support("dump"))echo"<a href='".h(ME)."dump=".urlencode(isset($_GET["table"])?$_GET["table"]:$_GET["select"])."' id='dump'".bold(isset($_GET["dump"])).">".'å¯¼å‡º'."</a>\n";}if($_GET["ns"]!==""&&!$me&&DB!=""){echo'<a href="'.h(ME).'create="'.bold($_GET["create"]==="").">".'åˆ›å»ºè¡¨'."</a>\n";$S=table_status('',true);if(!$S)echo"<p class='message'>".'æ²¡æœ‰è¡¨ã€‚'."\n";else{$this->tablesPrint($S);$Rd=array();foreach($S
...
<p id="dbs">
';hidden_fields_get();$Db=" onmousedown='dbMouseDown(event, this);' onchange='dbChange(this);'";echo"<span title='".'æ•°æ®åº“'."'>DB</span>: ".($j?"<select name='db'$Db>".optionlist(array(""=>"")+$j,DB)."</select>":'<input name="db" value="'.h(DB).'" autocapitalize="off">'),"<input type='submit' value='".'ä½¿ç”¨'."'".($j?" class='hidden'":"").">\n";if($me!="db"&&DB!=""&&$g->select_db(DB)){if(support("scheme")){echo"<br><select name='ns'$Db>".optionlist(array(""=>"(".'æ¨¡å¼'.")")+$b->schemas(),$_GET["ns"])."</select>";if($_GET["ns"]!="")set_schema($_GET["ns"]);}}echo(isset($_GET["sql"])?'<input type="hidden" name="sql" value="">':(isset($_GET["schema"])?'<input type="hidden" name="schema" value="">':(isset($_GET["dump"])?'<input type="hidden" name="dump" value="">':(isset($_GET["privileges"])?'<input type="hidden" name="privileges" value="">':"")))),"</p></form>\n";}function
tablesPrint($S){echo"<p id='tables' onmouseover='menuOver(this, event);' onmouseout='menuOut(this);'>\n";foreach($S
as$Q=>$xg){echo'<a href="'.h(ME).'select='.urlencode($Q).'"'.bold($_GET["select"]==$Q||$_GET["edit"]==$Q).">".'é€‰æ‹©'."</a> ";$C=$this->tableName($xg);echo(support("table")?'<a href="'.h(ME).'table='.urlencode($Q).'"'.bold(in_array($Q,array($_GET["table"],$_GET["create"],$_GET["indexes"],$_GET["foreign"],$_GET["trigger"])),(is_view($xg)?"view":""))." title='".'æ˜¾ç¤ºç»“æ„'."'>$C</a>":"<span>$C</span>")."<br>\n";}}}$b=(function_exists('adminer_object')?adminer_object():new
...
<p class="logout">
<input type="submit" name="logout" value="æ³¨é”€" id="logout">
...
long2str($W,true);}$g='';$Zc=$_SESSION["token"];if(!$Zc)$_SESSION["token"]=rand(1,1e6);$T=get_token();$if=array();if($_COOKIE["adminer_permanent"]){foreach(explode(" ",$_COOKIE["adminer_permanent"])as$X){list($x)=explode(":",$X);$if[$x]=$X;}}$Ia=$_POST["auth"];if($Ia){session_regenerate_id();$Sb=$Ia["driver"];$N=$Ia["server"];$V=$Ia["username"];$G=$Ia["password"];$k=$Ia["db"];set_password($Sb,$N,$V,$G);$_SESSION["db"][$Sb][$N][$V][$k]=true;if($Ia["permanent"]){$x=base64_encode($Sb)."-".base64_encode($N)."-".base64_encode($V)."-".base64_encode($k);$vf=$b->permanentLogin(true);$if[$x]="$x:".base64_encode($vf?encrypt_string($G,$vf):"");cookie("adminer_permanent",implode(" ",$if));}if(count($_POST)==1||DRIVER!=$Sb||SERVER!=$N||$_GET["username"]!==$V||DB!=$k)redirect(auth_url($Sb,$N,$V,$k));}elseif($_POST["logout"]){if($Zc&&!verify_token()){page_header('æ³¨é”€','æ— æ•ˆ CSRF ä»¤ç‰Œã€‚é‡æ–°å‘é€è¡¨å•ã€‚');page_footer("db");exit;}else{foreach(array("pwds","db","dbs","queries")as$x)set_session($x,null);unset_permanent();redirect(substr(preg_replace('~\b(username|db|ns)=[^&]*&~','',ME),0,-1),'æ³¨é”€æˆåŠŸã€‚');}}elseif($if&&!$_SESSION["pwds"]){session_regenerate_id();$vf=$b->permanentLogin();foreach($if
...
auth_error($sc=null){global$g,$b,$Zc;$og=session_name();$l="";if(!$_COOKIE[$og]&&$_GET[$og]&&ini_bool("session.use_only_cookies"))$l='Session å¿…é¡»è¢«å¯ç”¨ã€‚';elseif(isset($_GET["username"])){if(($_COOKIE[$og]||$_GET[$og])&&!$Zc)$l='Session å·²è¿‡æœŸï¼Œè¯·é‡æ–°ç™»å½•ã€‚';else{$G=get_password();if($G!==null){$l=h($sc?$sc->getMessage():(is_string($g)?$g:'æ— æ•ˆå‡­æ®ã€‚'));if($G===false)$l.='<br>'.sprintf('Master password expired. <a href="http://www.adminer.org/en/extension/" target="_blank">Implement</a> %s method to make it permanent.','<code>permanentLogin()</code>');set_password(DRIVER,SERVER,$_GET["username"],null);}unset_permanent();}}$F=session_get_cookie_params();cookie("adminer_key",($_COOKIE["adminer_key"]?$_COOKIE["adminer_key"]:rand_string()),$F["lifetime"]);page_header('ç™»å½•',$l,null);echo"<form action='' method='post'>\n";$b->loginForm();echo"<div>";hidden_fields($_POST,array("auth"));echo"</div>\n","</form>\n";page_footer("auth");}function
set_password($Jh,$N,$V,$G){$_SESSION["pwds"][$Jh][$N][$V]=($_COOKIE["adminer_key"]?array(encrypt_string($G,$_COOKIE["adminer_key"])):$G);}function
get_password(){$J=get_session("pwds");if(is_array($J))$J=($_COOKIE["adminer_key"]?decrypt_string($J[0],$_COOKIE["adminer_key"]):false);return$J;}if(isset($_GET["username"])){if(!class_exists("Min_DB")){unset($_SESSION["pwds"][DRIVER]);unset_permanent();page_header('æ²¡æœ‰æ‰©å±•',sprintf('æ²¡æœ‰æ”¯æŒçš„ PHP æ‰©å±•å¯ç”¨ï¼ˆ%sï¼‰ã€‚',implode(", ",$pf)),false);page_footer("auth");exit;}$g=connect();}if(is_string($g)||!$b->login($_GET["username"],get_password())){auth_error();exit;}$Sb=new
Min_Driver($g);if($Ia&&$_POST["token"])$_POST["token"]=$T;$l='';if($_POST){if(!verify_token()){$pd="max_input_vars";$ce=ini_get($pd);if(extension_loaded("suhosin")){foreach(array("suhosin.request.max_vars","suhosin.post.max_vars")as$x){$X=ini_get($x);if($X&&(!$ce||$X<$ce)){$pd=$x;$ce=$X;}}}$l=(!$_POST["token"]&&$ce?sprintf('è¶…è¿‡æœ€å¤šå…è®¸çš„å­—æ®µæ•°é‡ã€‚è¯·å¢åŠ  %sã€‚',"'$pd'"):'æ— æ•ˆ CSRF ä»¤ç‰Œã€‚é‡æ–°å‘é€è¡¨å•ã€‚');}}elseif($_SERVER["REQUEST_METHOD"]=="POST"){$l=sprintf('å¤ªå¤§çš„ POST æ•°æ®ã€‚å‡å°‘æ•°æ®æˆ–è€…å¢åŠ  %s é…ç½®å‘½ä»¤ã€‚',"'post_max_size'");if(isset($_GET["sql"]))$l.=' '.'You can upload a big SQL file via FTP and import it from server.';}if(!ini_bool("session.use_cookies")||@ini_set("session.use_cookies",false)!==false)session_write_close();function
connect_error(){global$b,$g,$T,$l,$Tb;$j=array();if(DB!=""){header("HTTP/1.1 404 Not Found");page_header('æ•°æ®åº“'.": ".h(DB),'æ— æ•ˆæ•°æ®åº“ã€‚',true);}else{if($_POST["db"]&&!$l)queries_redirect(substr(ME,0,-1),'å·²åˆ é™¤æ•°æ®åº“ã€‚',drop_databases($_POST["db"]));page_header('é€‰æ‹©æ•°æ®åº“',$l,false);echo"<p class='links'>\n";foreach(array('database'=>'åˆ›å»ºæ–°æ•°æ®åº“','privileges'=>'æƒé™','processlist'=>'è¿›ç¨‹åˆ—è¡¨','variables'=>'å˜é‡','status'=>'çŠ¶æ€',)as$x=>$X){if(support($x))echo"<a href='".h(ME)."$x='>$X</a>\n";}echo"<p>".sprintf('%s ç‰ˆæœ¬ï¼š%sï¼Œ ä½¿ç”¨PHPæ‰©å±• %s',$Tb[DRIVER],"<b>$g->server_info</b>","<b>$g->extension</b>")."\n","<p>".sprintf('ç™»å½•ç”¨æˆ·ï¼š%s',"<b>".h(logged_user())."</b>")."\n";$j=$b->databases();if($j){$eg=support("scheme");$ib=collations();echo"<form action='' method='post'>\n","<table cellspacing='0' class='checkable' onclick='tableClick(event);' ondblclick='tableClick(event, true);'>\n","<thead><tr>".(support("database")?"<td>&nbsp;":"")."<th>".'æ•°æ®åº“'."<td>".'æ ¡å¯¹'."<td>".'è¡¨'."</thead>\n";foreach($j
as$k){$Wf=h(ME)."db=".urlencode($k);echo"<tr".odd().">".(support("database")?"<td>".checkbox("db[]",$k,in_array($k,(array)$_POST["db"])):""),"<th><a href='$Wf'>".h($k)."</a>";$d=nbsp(db_collation($k,$ib));echo"<td>".(support("database")?"<a href='$Wf".($eg?"&amp;ns=":"")."&amp;database=' title='".'ä¿®æ”¹æ•°æ®åº“'."'>$d</a>":$d),"<td align='right'><a href='$Wf&amp;schema=' id='tables-".h($k)."' title='".'æ•°æ®åº“æ¦‚è¦'."'>?</a>","\n";}echo"</table>\n",(support("database")?"<fieldset><legend>".'Selected'." <span id='selected'></span></legend><div>\n"."<input type='hidden' name='all' value='' onclick=\"selectCount('selected', formChecked(this, /^db/));\">\n"."<input type='submit' name='drop' value='".'åˆ é™¤'."'".confirm().">\n"."</div></fieldset>\n":""),"<script type='text/javascript'>tableCheck();</script>\n","<input type='hidden' name='token' value='$T'>\n","</form>\n";}echo"<p><a href='".h(ME)."refresh=1'>".'åˆ·æ–°'."</a>\n";}page_footer("db");if($j)echo"<script type='text/javascript'>ajaxSetHtml('".js_escape(ME)."script=connect');</script>\n";}if(isset($_GET["status"]))$_GET["variables"]=$_GET["status"];if(isset($_GET["import"]))$_GET["sql"]=$_GET["import"];if(!(DB!=""?$g->select_db(DB):isset($_GET["sql"])||isset($_GET["dump"])||isset($_GET["database"])||isset($_GET["processlist"])||isset($_GET["privileges"])||isset($_GET["user"])||isset($_GET["variables"])||$_GET["script"]=="connect"||$_GET["script"]=="kill")){if(DB!=""||$_GET["refresh"]){restart_session();set_session("dbs",null);}connect_error();exit;}if(support("scheme")&&DB!=""&&$_GET["ns"]!==""){if(!isset($_GET["ns"]))redirect(preg_replace('~ns=[^&]*&~','',ME)."ns=".get_schema());if(!set_schema($_GET["ns"])){header("HTTP/1.1 404 Not Found");page_header('æ¨¡å¼'.": ".h($_GET["ns"]),'éæ³•æ¨¡å¼ã€‚',true);page_footer("ns");exit;}}function
...
as$x=>$X){if($X===null)$X="<i>NULL</i>";elseif($Ra[$x]&&!is_utf8($X))$X="<i>".sprintf('%d å­—èŠ‚',strlen($X))."</i>";elseif(!strlen($X))$X="&nbsp;";else{$X=h($X);if($rh[$x]==254)$X="<code>$X</code>";}if(isset($Rd[$x])&&!$f[$Rd[$x]]){if($Pe&&$w=="sql"){$Q=$K[array_search("table=",$Rd)];$_=$Rd[$x].urlencode($Pe[$Q]!=""?$Pe[$Q]:$Q);}else{$_="edit=".urlencode($Rd[$x]);foreach($v[$Rd[$x]]as$gb=>$zd)$_.="&where".urlencode("[".bracket_escape($gb)."]")."=".urlencode($K[$zd]);}$X="<a href='".h(ME.$_)."'>$X</a>";}echo"<td>$X";}}echo($r?"</table>":"<p class='message'>".'æ— æ•°æ®ã€‚')."\n";return$J;}function
...
edit_type($x,$m,$ib,$Lc=array()){global$_g,$rh,$yh,$De;$U=$m["type"];echo'<td><select name="',$x,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);"',on_help("getTarget(event).value",1),'>';if($U&&!isset($rh[$U])&&!isset($Lc[$U]))array_unshift($_g,$U);if($Lc)$_g['å¤–é”®']=$Lc;echo
optionlist($_g,$U),'</select>
<td><input name="',$x,'[length]" value="',h($m["length"]),'" size="3" onfocus="editingLengthFocus(this);"',(!$m["length"]&&preg_match('~var(char|binary)$~',$U)?" class='required'":""),' onchange="editingLengthChange(this);" onkeyup="this.onchange();"><td class="options">';echo"<select name='$x"."[collation]'".(preg_match('~(char|text|enum|set)$~',$U)?"":" class='hidden'").'><option value="">('.'æ ¡å¯¹'.')'.optionlist($ib,$m["collation"]).'</select>',($yh?"<select name='$x"."[unsigned]'".(!$U||preg_match('~((^|[^o])int|float|double|decimal)$~',$U)?"":" class='hidden'").'><option>'.optionlist($yh,$m["unsigned"]).'</select>':''),(isset($m['on_update'])?"<select name='$x"."[on_update]'".($U=="timestamp"?"":" class='hidden'").'>'.optionlist(array(""=>"(".'ON UPDATE'.")","CURRENT_TIMESTAMP"),$m["on_update"]).'</select>':''),($Lc?"<select name='$x"."[on_delete]'".(preg_match("~`~",$U)?"":" class='hidden'")."><option value=''>(".'ON DELETE'.")".optionlist(explode("|",$De),$m["on_delete"])."</select> ":" ");}function
...
';if($U=="PROCEDURE"){echo'<td>&nbsp;';}echo'<th>',($U=="TABLE"?'å­—æ®µå':'å‚æ•°å'),'<td>ç±»å‹<textarea id="enum-edit" rows="4" cols="12" wrap="off" style="display: none;" onblur="editingLengthBlur(this);"></textarea>
<td>é•¿åº¦
<td>é€‰é¡¹
';if($U=="TABLE"){echo'<td>NULL
<td><input type="radio" name="auto_increment_col" value=""><acronym title="è‡ªåŠ¨å¢é‡">AI</acronym>',doc_link(array('sql'=>"example-auto-increment.html",'sqlite'=>"autoinc.html",'pgsql'=>"datatype.html#DATATYPE-SERIAL",'mssql'=>"ms186775.aspx",)),'<td>é»˜è®¤å€¼
',(support("comment")?"<td".($nb?"":" class='hidden'").">".'æ³¨é‡Š':"");}echo'<td>',"<input type='image' class='icon' name='add[".(support("move_col")?0:count($n))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=4.0.0' alt='+' title='".'ä¸‹ä¸€è¡Œæ’å…¥'."'>",'<script type="text/javascript">row_count = ',count($n),';</script>
</thead>
...
',(support("comment")?"<td".($nb?"":" class='hidden'")."><input name='fields[$r][comment]' value='".h($m["comment"])."' maxlength='".($g->server_info>=5.5?1024:255)."'>":"");}echo"<td>",(support("move_col")?"<input type='image' class='icon' name='add[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=4.0.0' alt='+' title='".'ä¸‹ä¸€è¡Œæ’å…¥'."' onclick='return !editingAddRow(this, 1);'>&nbsp;"."<input type='image' class='icon' name='up[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=4.0.0' alt='^' title='".'ä¸Šç§»'."'>&nbsp;"."<input type='image' class='icon' name='down[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=4.0.0' alt='v' title='".'ä¸‹ç§»'."'>&nbsp;":""),($Qe==""||support("drop_col")?"<input type='image' class='icon' name='drop_col[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=4.0.0' alt='x' title='".'ç§»é™¤'."' onclick=\"return !editingRemoveRow(this, 'fields\$1[field]');\">":""),"\n";}}function
...
<p class="links"><a href="',h(ME."schema=".urlencode($ea)),'" id="schema-link">å›ºå®šé“¾æ¥</a>
...
pack("x512");}}}if($vd)echo"-- ".$g->result("SELECT NOW()")."\n";exit;}page_header('å¯¼å‡º',$l,($_GET["export"]!=""?array("table"=>$_GET["export"]):array()),h(DB));echo'
<form action="" method="post">
<table cellspacing="0">
';$Eb=array('','USE','DROP+CREATE','CREATE');$Lg=array('','DROP+CREATE','CREATE');$Bb=array('','TRUNCATE+INSERT','INSERT');if($w=="sql")$Bb[]='INSERT+UPDATE';parse_str($_COOKIE["adminer_export"],$K);if(!$K)$K=array("output"=>"text","format"=>"sql","db_style"=>(DB!=""?"":"CREATE"),"table_style"=>"DROP+CREATE","data_style"=>"INSERT");if(!isset($K["events"])){$K["routines"]=$K["events"]=($_GET["dump"]=="");$K["triggers"]=$K["table_style"];}echo"<tr><th>".'è¾“å‡º'."<td>".html_select("output",$b->dumpOutput(),$K["output"],0)."\n";echo"<tr><th>".'æ ¼å¼'."<td>".html_select("format",$b->dumpFormat(),$K["format"],0)."\n";echo($w=="sqlite"?"":"<tr><th>".'æ•°æ®åº“'."<td>".html_select('db_style',$Eb,$K["db_style"]).(support("routine")?checkbox("routines",1,$K["routines"],'å­ç¨‹åº'):"").(support("event")?checkbox("events",1,$K["events"],'äº‹ä»¶'):"")),"<tr><th>".'è¡¨'."<td>".html_select('table_style',$Lg,$K["table_style"]).checkbox("auto_increment",1,$K["auto_increment"],'è‡ªåŠ¨å¢é‡').(support("trigger")?checkbox("triggers",1,$K["triggers"],'è§¦å‘å™¨'):""),"<tr><th>".'æ•°æ®'."<td>".html_select('data_style',$Bb,$K["data_style"]),'</table>
<p><input type="submit" value="å¯¼å‡º">
<input type="hidden" name="token" value="',$T,'">

<table cellspacing="0">
';$rf=array();if(DB!=""){$Za=($a!=""?"":" checked");echo"<thead><tr>","<th style='text-align: left;'><label class='block'><input type='checkbox' id='check-tables'$Za onclick='formCheck(this, /^tables\\[/);'>".'è¡¨'."</label>","<th style='text-align: right;'><label class='block'>".'æ•°æ®'."<input type='checkbox' id='check-data'$Za onclick='formCheck(this, /^data\\[/);'></label>","</thead>\n";$Mh="";$Mg=tables_list();foreach($Mg
...
as$C=>$m){if(!isset($m["privileges"][$zh?"update":"insert"])||$b->fieldName($m)=="")unset($n[$C]);}if($_POST&&!$l&&!isset($_GET["select"])){$A=$_POST["referer"];if($_POST["insert"])$A=($zh?null:$_SERVER["REQUEST_URI"]);elseif(!preg_match('~^.+&select=.+$~',$A))$A=ME."select=".urlencode($a);$v=indexes($a);$uh=unique_array($_GET["where"],$v);$Df="\nWHERE $Z";if(isset($_POST["delete"]))queries_redirect($A,'å·²åˆ é™¤é¡¹ç›®ã€‚',$Sb->delete($a,$Df,!$uh));else{$O=array();foreach($n
as$C=>$m){$X=process_input($m);if($X!==false&&$X!==null)$O[idf_escape($C)]=$X;}if($zh){if(!$O)redirect($A);queries_redirect($A,'å·²æ›´æ–°é¡¹ç›®ã€‚',$Sb->update($a,$O,$Df,!$uh));if(is_ajax()){page_headers();page_messages($l);exit;}}else{$I=$Sb->insert($a,$O);$Jd=($I?last_id():0);queries_redirect($A,sprintf('å·²æ’å…¥é¡¹ç›®%sã€‚',($Jd?" $Jd":"")),$I);}}}$Hg=$b->tableName(table_status1($a,true));page_header(($zh?'ç¼–è¾‘':'æ’å…¥'),$l,array("select"=>array($a,$Hg)),$Hg);$K=null;if($_POST["save"])$K=(array)$_POST["fields"];elseif($Z){$M=array();foreach($n
...
as$x=>$X){if(!$Z)$K[$x]=null;$n[$x]=array("field"=>$x,"null"=>($x!="itemName()"),"auto_increment"=>($x=="itemName()"));}}}if($K===false)echo"<p class='error'>".'æ— æ•°æ®ã€‚'."\n";echo'
<div id="message"></div>
...
';if($n){echo"<input type='submit' value='".'ä¿å­˜'."'>\n";if(!isset($_GET["select"]))echo"<input type='submit' name='insert' value='".($zh?'ä¿å­˜å¹¶ç»§ç»­ç¼–è¾‘'."' onclick='return !ajaxForm(this.form, \"".'Saving'.'...", this)':'ä¿å­˜å¹¶æ’å…¥ä¸‹ä¸€ä¸ª')."' title='Ctrl+Shift+Enter'>\n";}echo($zh?"<input type='submit' name='delete' value='".'åˆ é™¤'."'".confirm().">\n":($_POST||!$n?"":"<script type='text/javascript'>focus(document.getElementById('form').getElementsByTagName('td')[1].firstChild);</script>\n"));if(isset($_GET["select"]))hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
...
<p>
';if(support("columns")||$a==""){echo'è¡¨å: <input name="name" maxlength="64" value="',h($K["name"]),'" autocapitalize="off">
';if($a==""&&!$_POST){?><script type='text/javascript'>focus(document.getElementById('form')['name']);</script><?php }echo($kc?"<select name='Engine' onchange='helpClose();'".on_help("getTarget(event).value",1).">".optionlist(array(""=>"(".'å¼•æ“'.")")+$kc,$K["Engine"])."</select>":""),' ',($ib&&!preg_match("~sqlite|mssql~",$w)?html_select("Collation",array(""=>"(".'æ ¡å¯¹'.")")+$ib,$K["Collation"]):""),' <input type="submit" value="ä¿å­˜">
';}echo'
...
<p>
è‡ªåŠ¨å¢é‡: <input type="number" name="Auto_increment" size="6" value="',h($K["Auto_increment"]),'">
',checkbox("defaults",1,true,'é»˜è®¤å€¼',"columnShow(this.checked, 5)","jsonly");if(!$_POST["defaults"]){echo'<script type="text/javascript">editingHideDefaults()</script>';}echo(support("comment")?"<label><input type='checkbox' name='comments' value='1' class='jsonly' onclick=\"columnShow(this.checked, 6); toggle('Comment'); if (this.checked) this.form['Comment'].focus();\"".($nb?" checked":"").">".'æ³¨é‡Š'."</label>".' <input name="Comment" id="Comment" value="'.h($K["Comment"]).'" maxlength="'.($g->server_info>=5.5?2048:60).'"'.($nb?'':' class="hidden"').'>':''),'<p>
<input type="submit" value="ä¿å­˜">
';}echo'
';if($a!=""){echo'<input type="submit" name="drop" value="åˆ é™¤"',confirm(),'>';}if(support("partitioning")){$bf=preg_match('~RANGE|LIST~',$K["partition_by"]);print_fieldset("partition",'åˆ†åŒºç±»å‹',$K["partition_by"]);echo'<p>
...
åˆ†åŒº: <input type="number" name="partitions" class="size',($bf||!$K["partition_by"]?" hidden":""),'" value="',h($K["partitions"]),'">
...
<th>åç§°
<th><noscript><input type='image' class='icon' name='add[0]' src='" . h(preg_replace("~\\?.*~", "", ME)) . "?file=plus.gif&amp;version=4.0.0' alt='+' title='ä¸‹ä¸€è¡Œæ’å…¥'></noscript>&nbsp;
</thead>
<?php
if($sf){echo"<tr><td>PRIMARY<td>";foreach($sf["columns"]as$x=>$e){echo"<select disabled>".optionlist($n,$e)."</select>","<label><input disabled type='checkbox'>".'é™åº'."</label> ";}echo"<td><td>\n";}$zd=1;foreach($K["indexes"]as$u){if(!$_POST["drop_col"]||$zd!=key($_POST["drop_col"])){echo"<tr><td>".html_select("indexes[$zd][type]",array(-1=>"")+$ld,$u["type"],($zd==count($K["indexes"])?"indexesAddRow(this);":1)),"<td>";ksort($u["columns"]);$r=1;foreach($u["columns"]as$x=>$e){echo"<span>".html_select("indexes[$zd][columns][$r]",array(-1=>"")+$n,$e,($r==count($u["columns"])?"indexesAddColumn":"indexesChangeColumn")."(this, '".js_escape($w=="sql"?"":$_GET["indexes"]."_")."');"),($w=="sql"||$w=="mssql"?"<input type='number' name='indexes[$zd][lengths][$r]' class='size' value='".h($u["lengths"][$x])."'>":""),($w!="sql"?checkbox("indexes[$zd][descs][$r]",1,$u["descs"][$x],'é™åº'):"")," </span>";$r++;}echo"<td><input name='indexes[$zd][name]' value='".h($u["name"])."' autocapitalize='off'>\n","<td><input type='image' class='icon' name='drop_col[$zd]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=4.0.0' alt='x' title='".'ç§»é™¤'."' onclick=\"return !editingRemoveRow(this, 'indexes\$1[type]');\">\n";}$zd++;}echo'</table>
<p>
<input type="submit" value="ä¿å­˜">
<input type="hidden" name="token" value="',$T,'">
</form>
';}elseif(isset($_GET["database"])){$K=$_POST;if($_POST&&!$l&&!isset($_POST["add_x"])){restart_session();$C=trim($K["name"]);if($_POST["drop"]){$_GET["db"]="";queries_redirect(remove_from_uri("db|database"),'å·²åˆ é™¤æ•°æ®åº“ã€‚',drop_databases(array(DB)));}elseif(DB!==$C){if(DB!=""){$_GET["db"]=$C;queries_redirect(preg_replace('~\bdb=[^&]*&~','',ME)."db=".urlencode($C),'å·²é‡å‘½åæ•°æ®åº“ã€‚',rename_database($C,$K["collation"]));}else{$j=explode("\n",str_replace("\r","",$C));$Bg=true;$Id="";foreach($j
as$k){if(count($j)==1||$k!=""){if(!create_database($k,$K["collation"]))$Bg=false;$Id=$k;}}queries_redirect(ME."db=".urlencode($Id),'å·²åˆ›å»ºæ•°æ®åº“ã€‚',$Bg);}}else{if(!$K["collation"])redirect(substr(ME,0,-1));query_redirect("ALTER DATABASE ".idf_escape($C).(preg_match('~^[a-z0-9_]+$~i',$K["collation"])?" COLLATE $K[collation]":""),substr(ME,0,-1),'å·²ä¿®æ”¹æ•°æ®åº“ã€‚');}}page_header(DB!=""?'ä¿®æ”¹æ•°æ®åº“':'åˆ›å»ºæ•°æ®åº“',$l,array(),h(DB));$ib=collations();$C=DB;if($_POST)$C=$K["name"];elseif(DB!="")$K["collation"]=db_collation(DB,$ib);elseif($w=="sql"){foreach(get_vals("SHOW GRANTS")as$Tc){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$Tc,$B)&&$B[1]){$C=stripcslashes(idf_unescape("`$B[2]`"));break;}}}echo'
<form action="" method="post">
<p>
',($_POST["add_x"]||strpos($C,"\n")?'<textarea id="name" name="name" rows="10" cols="40">'.h($C).'</textarea><br>':'<input name="name" id="name" value="'.h($C).'" maxlength="64" autocapitalize="off">')."\n".($ib?html_select("collation",array(""=>"(".'æ ¡å¯¹'.")")+$ib,$K["collation"]).doc_link(array('sql'=>"charset-charsets.html",'mssql'=>"ms187963.aspx",)):"");?>
<script type='text/javascript'>focus(document.getElementById('name'));</script>
<input type="submit" value="ä¿å­˜">
<?php
if(DB!="")echo"<input type='submit' name='drop' value='".'åˆ é™¤'."'".confirm().">\n";elseif(!$_POST["add_x"]&&$_GET["db"]=="")echo"<input type='image' class='icon' name='add' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=4.0.0' alt='+' title='".'ä¸‹ä¸€è¡Œæ’å…¥'."'>\n";echo'<input type="hidden" name="token" value="',$T,'">
</form>
';}elseif(isset($_GET["scheme"])){$K=$_POST;if($_POST&&!$l){$_=preg_replace('~ns=[^&]*&~','',ME)."ns=";if($_POST["drop"])query_redirect("DROP SCHEMA ".idf_escape($_GET["ns"]),$_,'å·²åˆ é™¤æ¨¡å¼ã€‚');else{$C=trim($K["name"]);$_.=urlencode($C);if($_GET["ns"]=="")query_redirect("CREATE SCHEMA ".idf_escape($C),$_,'å·²åˆ›å»ºæ¨¡å¼ã€‚');elseif($_GET["ns"]!=$C)query_redirect("ALTER SCHEMA ".idf_escape($_GET["ns"])." RENAME TO ".idf_escape($C),$_,'å·²ä¿®æ”¹æ¨¡å¼ã€‚');else
...
';}if($C!=""){echo'<input type="submit" name="drop" value="åˆ é™¤"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$T,'">
</form>
';}elseif(isset($_GET["view"])){$a=$_GET["view"];$K=$_POST;if($_POST&&!$l){$C=trim($K["name"]);$Ea=" AS\n$K[select]";$A=ME."table=".urlencode($C);$ee='å·²ä¿®æ”¹è§†å›¾ã€‚';if(!$_POST["drop"]&&$a==$C&&$w!="sqlite")query_redirect(($w=="mssql"?"ALTER":"CREATE OR REPLACE")." VIEW ".table($C).$Ea,$A,$ee);else{$Rg=$C."_adminer_".uniqid();drop_create("DROP VIEW ".table($a),"CREATE VIEW ".table($C).$Ea,"DROP VIEW ".table($C),"CREATE VIEW ".table($Rg).$Ea,"DROP VIEW ".table($Rg),($_POST["drop"]?substr(ME,0,-1):$A),'å·²åˆ é™¤è§†å›¾ã€‚',$ee,'å·²åˆ›å»ºè§†å›¾ã€‚',$a,$C);}}if(!$_POST&&$a!=""){$K=view($a);$K["name"]=$a;if(!$l)$l=$g->error;}page_header(($a!=""?'ä¿®æ”¹è§†å›¾':'åˆ›å»ºè§†å›¾'),$l,array("table"=>$a),h($a));echo'
<form action="" method="post">
<p>åç§°: <input name="name" value="',h($K["name"]),'" maxlength="64" autocapitalize="off">
<p>';textarea("select",$K["select"]);echo'<p>
<input type="submit" value="ä¿å­˜">
';if($_GET["view"]!=""){echo'<input type="submit" name="drop" value="åˆ é™¤"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$T,'">
</form>
';}elseif(isset($_GET["event"])){$aa=$_GET["event"];$td=array("YEAR","QUARTER","MONTH","DAY","HOUR","MINUTE","WEEK","SECOND","YEAR_MONTH","DAY_HOUR","DAY_MINUTE","DAY_SECOND","HOUR_MINUTE","HOUR_SECOND","MINUTE_SECOND");$yg=array("ENABLED"=>"ENABLE","DISABLED"=>"DISABLE","SLAVESIDE_DISABLED"=>"DISABLE ON SLAVE");$K=$_POST;if($_POST&&!$l){if($_POST["drop"])query_redirect("DROP EVENT ".idf_escape($aa),substr(ME,0,-1),'å·²åˆ é™¤äº‹ä»¶ã€‚');elseif(in_array($K["INTERVAL_FIELD"],$td)&&isset($yg[$K["STATUS"]])){$cg="\nON SCHEDULE ".($K["INTERVAL_VALUE"]?"EVERY ".q($K["INTERVAL_VALUE"])." $K[INTERVAL_FIELD]".($K["STARTS"]?" STARTS ".q($K["STARTS"]):"").($K["ENDS"]?" ENDS ".q($K["ENDS"]):""):"AT ".q($K["STARTS"]))." ON COMPLETION".($K["ON_COMPLETION"]?"":" NOT")." PRESERVE";queries_redirect(substr(ME,0,-1),($aa!=""?'å·²ä¿®æ”¹äº‹ä»¶ã€‚':'å·²åˆ›å»ºäº‹ä»¶ã€‚'),queries(($aa!=""?"ALTER EVENT ".idf_escape($aa).$cg.($aa!=$K["EVENT_NAME"]?"\nRENAME TO ".idf_escape($K["EVENT_NAME"]):""):"CREATE EVENT ".idf_escape($K["EVENT_NAME"]).$cg)."\n".$yg[$K["STATUS"]]." COMMENT ".q($K["EVENT_COMMENT"]).rtrim(" DO\n$K[EVENT_DEFINITION]",";").";"));}}page_header(($aa!=""?'ä¿®æ”¹äº‹ä»¶'.": ".h($aa):'åˆ›å»ºäº‹ä»¶'),$l);if(!$K&&$aa!=""){$L=get_rows("SELECT * FROM information_schema.EVENTS WHERE EVENT_SCHEMA = ".q(DB)." AND EVENT_NAME = ".q($aa));$K=reset($L);}echo'
<form action="" method="post">
<table cellspacing="0">
...
<tr><th>æ¯<td><input type="number" name="INTERVAL_VALUE" value="',h($K["INTERVAL_VALUE"]),'" class="size"> ',html_select("INTERVAL_FIELD",$td,$K["INTERVAL_FIELD"]),'<tr><th>çŠ¶æ€<td>',html_select("STATUS",$yg,$K["STATUS"]),'<tr><th>æ³¨é‡Š<td><input name="EVENT_COMMENT" value="',h($K["EVENT_COMMENT"]),'" maxlength="64">
<tr><th>&nbsp;<td>',checkbox("ON_COMPLETION","PRESERVE",$K["ON_COMPLETION"]=="PRESERVE",'å®Œæˆåä¿å­˜'),'</table>
<p>';textarea("EVENT_DEFINITION",$K["EVENT_DEFINITION"]);echo'<p>
<input type="submit" value="ä¿å­˜">
';if($aa!=""){echo'<input type="submit" name="drop" value="åˆ é™¤"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$T,'">
</form>
';}elseif(isset($_GET["procedure"])){$da=$_GET["procedure"];$Xf=(isset($_GET["function"])?"FUNCTION":"PROCEDURE");$K=$_POST;$K["fields"]=(array)$K["fields"];if($_POST&&!process_fields($K["fields"])&&!$l){$Rg="$K[name]_adminer_".uniqid();drop_create("DROP $Xf ".idf_escape($da),create_routine($Xf,$K),"DROP $Xf ".idf_escape($K["name"]),create_routine($Xf,array("name"=>$Rg)+$K),"DROP $Xf ".idf_escape($Rg),substr(ME,0,-1),'å·²åˆ é™¤å­ç¨‹åºã€‚','å·²ä¿®æ”¹å­ç¨‹åºã€‚','å·²åˆ›å»ºå­ç¨‹åºã€‚',$da,$K["name"]);}page_header(($da!=""?(isset($_GET["function"])?'ä¿®æ”¹å‡½æ•°':'ä¿®æ”¹è¿‡ç¨‹').": ".h($da):(isset($_GET["function"])?'åˆ›å»ºå‡½æ•°':'åˆ›å»ºè¿‡ç¨‹')),$l);if(!$_POST&&$da!=""){$K=routine($da,$Xf);$K["name"]=$da;}$ib=get_vals("SHOW CHARACTER SET");sort($ib);$Yf=routine_languages();echo'
<form action="" method="post" id="form">
<p>åç§°: <input name="name" value="',h($K["name"]),'" maxlength="64" autocapitalize="off">
',($Yf?'è¯­è¨€'.": ".html_select("language",$Yf,$K["language"]):""),'<input type="submit" value="ä¿å­˜">
<table cellspacing="0" class="nowrap">
';edit_fields($K["fields"],$ib,$Xf);if(isset($_GET["function"])){echo"<tr><td>".'è¿”å›ç±»å‹';edit_type("returns",$K["returns"],$ib);}echo'</table>
<p>';textarea("definition",$K["definition"]);echo'<p>
<input type="submit" value="ä¿å­˜">
';if($da!=""){echo'<input type="submit" name="drop" value="åˆ é™¤"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$T,'">
</form>
';}elseif(isset($_GET["sequence"])){$fa=$_GET["sequence"];$K=$_POST;if($_POST&&!$l){$_=substr(ME,0,-1);$C=trim($K["name"]);if($_POST["drop"])query_redirect("DROP SEQUENCE ".idf_escape($fa),$_,'å·²åˆ é™¤åºåˆ—ã€‚');elseif($fa=="")query_redirect("CREATE SEQUENCE ".idf_escape($C),$_,'å·²åˆ›å»ºåºåˆ—ã€‚');elseif($fa!=$C)query_redirect("ALTER SEQUENCE ".idf_escape($fa)." RENAME TO ".idf_escape($C),$_,'å·²ä¿®æ”¹åºåˆ—ã€‚');else
redirect($_);}page_header($fa!=""?'ä¿®æ”¹åºåˆ—'.": ".h($fa):'åˆ›å»ºåºåˆ—',$l);if(!$K)$K["name"]=$fa;echo'
<form action="" method="post">
<p><input name="name" value="',h($K["name"]),'" autocapitalize="off">
<input type="submit" value="ä¿å­˜">
';if($fa!="")echo"<input type='submit' name='drop' value='".'åˆ é™¤'."'".confirm().">\n";echo'<input type="hidden" name="token" value="',$T,'">
</form>
';}elseif(isset($_GET["type"])){$ga=$_GET["type"];$K=$_POST;if($_POST&&!$l){$_=substr(ME,0,-1);if($_POST["drop"])query_redirect("DROP TYPE ".idf_escape($ga),$_,'å·²åˆ é™¤ç±»å‹ã€‚');else
query_redirect("CREATE TYPE ".idf_escape(trim($K["name"]))." $K[as]",$_,'å·²åˆ›å»ºç±»å‹ã€‚');}page_header($ga!=""?'ä¿®æ”¹ç±»å‹'.": ".h($ga):'åˆ›å»ºç±»å‹',$l);if(!$K)$K["as"]="AS ";echo'
<form action="" method="post">
<p>
';if($ga!="")echo"<input type='submit' name='drop' value='".'åˆ é™¤'."'".confirm().">\n";else{echo"<input name='name' value='".h($K['name'])."' autocapitalize='off'>\n";textarea("as",$K["as"]);echo"<p><input type='submit' value='".'ä¿å­˜'."'>\n";}echo'<input type="hidden" name="token" value="',$T,'">
</form>
';}elseif(isset($_GET["trigger"])){$a=$_GET["trigger"];$C=$_GET["name"];$nh=trigger_options();$lh=array("INSERT","UPDATE","DELETE");$K=(array)trigger($C)+array("Trigger"=>$a."_bi");if($_POST){if(!$l&&in_array($_POST["Timing"],$nh["Timing"])&&in_array($_POST["Event"],$lh)&&in_array($_POST["Type"],$nh["Type"])){$Ce=" ON ".table($a);$Ub="DROP TRIGGER ".idf_escape($C).($w=="pgsql"?$Ce:"");$A=ME."table=".urlencode($a);if($_POST["drop"])query_redirect($Ub,$A,'å·²åˆ é™¤è§¦å‘å™¨ã€‚');else{if($C!="")queries($Ub);queries_redirect($A,($C!=""?'å·²ä¿®æ”¹è§¦å‘å™¨ã€‚':'å·²åˆ›å»ºè§¦å‘å™¨ã€‚'),queries(create_trigger($Ce,$_POST)));if($C!="")queries(create_trigger($Ce,$K+array("Type"=>reset($nh["Type"]))));}}$K=$_POST;}page_header(($C!=""?'ä¿®æ”¹è§¦å‘å™¨'.": ".h($C):'åˆ›å»ºè§¦å‘å™¨'),$l,array("table"=>$a));echo'
<form action="" method="post" id="form">
<table cellspacing="0">
<tr><th>æ—¶é—´<td>',html_select("Timing",$nh["Timing"],$K["Timing"],"if (/^".preg_quote($a,"/")."_[ba][iud]$/.test(this.form['Trigger'].value)) this.form['Trigger'].value = '".js_escape($a)."_' + selectValue(this).charAt(0).toLowerCase() + selectValue(this.form['Event']).charAt(0).toLowerCase();"),'<tr><th>äº‹ä»¶<td>',html_select("Event",$lh,$K["Event"],"this.form['Timing'].onchange();"),'<tr><th>ç±»å‹<td>',html_select("Type",$nh["Type"],$K["Type"]),'</table>
...
as$xe=>$Uf){if(preg_match('~^(.+)(\\(.*\\))?$~U',$xe,$B))grant("REVOKE",array_keys($Uf),$B[2]," ON $B[1] FROM $te");}}}queries_redirect(ME."privileges=",(isset($_GET["host"])?'å·²ä¿®æ”¹ç”¨æˆ·ã€‚':'å·²åˆ›å»ºç”¨æˆ·ã€‚'),!$l);if($zb)$g->query("DROP USER $te");}}page_header((isset($_GET["host"])?'ç”¨æˆ·å'.": ".h("$ha@$_GET[host]"):'åˆ›å»ºç”¨æˆ·'),$l,array("privileges"=>array('','æƒé™')));if($_POST){$K=$_POST;$Uc=$re;}else{$K=$_GET+array("host"=>$g->result("SELECT SUBSTRING_INDEX(CURRENT_USER, '@', -1)"));$K["pass"]=$Ae;if($Ae!="")$K["hashed"]=true;$Uc[(DB==""||$Uc?"":idf_escape(addcslashes(DB,"%_\\"))).".*"]=array();}echo'<form action="" method="post">
<table cellspacing="0">
<tr><th>æœåŠ¡å™¨<td><input name="host" maxlength="60" value="',h($K["host"]),'" autocapitalize="off">
<tr><th>ç”¨æˆ·å<td><input name="user" maxlength="16" value="',h($K["user"]),'" autocapitalize="off">
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'>".'æƒé™'.doc_link(array('sql'=>"grant.html#priv_level"));$r=0;foreach($Uc
as$xe=>$Tc){echo'<th>'.($xe!="*.*"?"<input name='objects[$r]' value='".h($xe)."' size='10' autocapitalize='off'>":"<input type='hidden' name='objects[$r]' value='*.*' size='10'>*.*");$r++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'æœåŠ¡å™¨',"Databases"=>'æ•°æ®åº“',"Tables"=>'è¡¨',"Columns"=>'åˆ—',"Procedures"=>'å­ç¨‹åº',)as$tb=>$Kb){foreach((array)$xf[$tb]as$wf=>$mb){echo"<tr".odd()."><td".($Kb?">$Kb<td":" colspan='2'").' lang="en" title="'.h($mb).'">'.h($wf);$r=0;foreach($Uc
as$xe=>$Tc){$C="'grants[$r][".h(strtoupper($wf))."]'";$Y=$Tc[strtoupper($wf)];if($tb=="Server Admin"&&$xe!=(isset($Uc["*.*"])?"*.*":".*"))echo"<td>&nbsp;";elseif(isset($_GET["grant"]))echo"<td><select name=$C><option><option value='1'".($Y?" selected":"").">".'æˆæƒ'."<option value='0'".($Y=="0"?" selected":"").">".'åºŸé™¤'."</select>";else
...
<input type="submit" value="ä¿å­˜">
';if(isset($_GET["host"])){echo'<input type="submit" name="drop" value="åˆ é™¤"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$T,'">
</form>
';}elseif(isset($_GET["processlist"])){if(support("kill")&&$_POST&&!$l){$Fd=0;foreach((array)$_POST["kill"]as$X){if(queries("KILL ".(+$X)))$Fd++;}queries_redirect(ME."processlist=",sprintf('%d ä¸ªè¿›ç¨‹è¢«ç»ˆæ­¢',$Fd),$Fd||!$_POST["kill"]);}page_header('è¿›ç¨‹åˆ—è¡¨',$l);echo'
<form action="" method="post">
...
as$x=>$X)echo"<td>".(($w=="sql"&&$x=="Info"&&preg_match("~Query|Killed~",$K["Command"])&&$X!="")||($w=="pgsql"&&$x=="current_query"&&$X!="<IDLE>")||($w=="oracle"&&$x=="sql_text"&&$X!="")?"<code class='jush-$w'>".shorten_utf8($X,100,"</code>").' <a href="'.h(ME.($K["db"]!=""?"db=".urlencode($K["db"])."&":"")."sql=".urlencode($X)).'">'.'å¤åˆ¶'.'</a>':nbsp($X));echo"\n";}echo'</table>
<script type=\'text/javascript\'>tableCheck();</script>
<p>
';if(support("kill")){echo($r+1)."/".sprintf('å…±è®¡ %d',$g->result("SELECT @@max_connections")),"<p><input type='submit' value='".'ç»ˆæ­¢'."'>\n";}echo'<input type="hidden" name="token" value="',$T,'">
</form>
...
as$_d=>$W)$_.=where_link($r++,$_d,$W);}$X=select_value($X,$_,$m,$Vg);$s=h("val[$vh][".bracket_escape($x)."]");$Y=$_POST["val"][$vh][bracket_escape($x)];$cc=!is_array($K[$x])&&is_utf8($X)&&$L[$pe][$x]==$K[$x]&&!$Sc[$x];$Ug=preg_match('~text|lob~',$m["type"]);if(($_GET["modify"]&&$cc)||$Y!==null){$Xc=h($Y!==null?$Y:$K[$x]);echo"<td>".($Ug?"<textarea name='$s' cols='30' rows='".(substr_count($K[$x],"\n")+1)."'>$Xc</textarea>":"<input name='$s' value='$Xc' size='$Od[$x]'>");}else{$Td=strpos($X,"<i>...</i>");echo"<td id='$s' onclick=\"selectClick(this, event, ".($Td?2:($Ug?1:0)).($cc?"":", '".h('ä½¿ç”¨ç¼–è¾‘é“¾æ¥æ¥ä¿®æ”¹è¯¥å€¼ã€‚')."'").");\">$X";}}}if($Na)echo"<td>";$b->backwardKeysPrint($Na,$L[$pe]);echo"</tr>\n";}if(is_ajax())exit;echo"</table>\n";}if(($L||$E)&&!is_ajax()){$rc=true;if($_GET["page"]!="last"){if(!+$z)$Oc=count($L);elseif($w!="sql"||!$ud){$Oc=($ud?false:found_rows($R,$Z));if($Oc<max(1e4,2*($E+1)*$z))$Oc=reset(slow_query(count_rows($a,$Z,$ud,$q)));else$rc=false;}}if(+$z&&($Oc===false||$Oc>$z||$E)){echo"<p class='pages'>";$Zd=($Oc===false?$E+(count($L)>=$z?2:1):floor(($Oc-1)/$z));if(support("table")){echo'<a href="'.h(remove_from_uri("page"))."\" onclick=\"pageClick(this.href, +prompt('".'é¡µé¢'."', '".($E+1)."'), event); return false;\">".'é¡µé¢'."</a>:",pagination(0,$E).($E>5?" ...":"");for($r=max(1,$E-4);$r<min($Zd,$E+5);$r++)echo
pagination($r,$E);if($Zd>0){echo($E+5<$Zd?" ...":""),($rc&&$Oc!==false?pagination($Zd,$E):" <a href='".h(remove_from_uri("page")."&page=last")."' title='~$Zd'>".'æœ€å'."</a>");}echo(($Oc===false?count($L)+1:$Oc-$E*$z)>$z?' <a href="'.h(remove_from_uri("page")."&page=".($E+1)).'" onclick="return !selectLoadMore(this, '.(+$z).', \''.'Loading'.'...\');">'.'Load more data'.'</a>':'');}else{echo'é¡µé¢'.":",pagination(0,$E).($E>1?" ...":""),($E?pagination($E,$E):""),($Zd>$E?pagination($E+1,$E).($Zd>$E+1?" ...":""):"");}}echo"<p class='count'>\n",($Oc!==false?"(".($rc?"":"~ ").sprintf('%d è¡Œ',$Oc).") ":"");$Qb=($rc?"":"~ ").$Oc;echo
checkbox("all",1,0,'æ‰€æœ‰ç»“æœ',"var checked = formChecked(this, /check/); selectCount('selected', this.checked ? '$Qb' : checked); selectCount('selected2', this.checked || !checked ? '$Qb' : checked);")."\n";if($b->selectCommandPrint()){echo'<fieldset',($_GET["modify"]?'':' class="jsonly"'),'><legend>Modify</legend><div>
<input type="submit" value="ä¿å­˜"',($_GET["modify"]?'':' title="'.'Ctrl+click on a value to modify it.'.'"'),'>
</div></fieldset>
<fieldset><legend>Selected <span id="selected"></span></legend><div>
<input type="submit" name="edit" value="ç¼–è¾‘">
<input type="submit" name="clone" value="å¤åˆ¶">
<input type="submit" name="delete" value="åˆ é™¤"',confirm(),'>
</div></fieldset>
';}$Mc=$b->dumpFormat();foreach((array)$_GET["columns"]as$e){if($e["fun"]){unset($Mc['sql']);break;}}if($Mc){print_fieldset("export",'å¯¼å‡º'." <span id='selected2'></span>");$Ve=$b->dumpOutput();echo($Ve?html_select("output",$Ve,$wa["output"])." ":""),html_select("format",$Mc,$wa["format"])," <input type='submit' name='export' value='".'å¯¼å‡º'."'>\n","</div></fieldset>\n";}echo(!$q&&$M?"":"<script type='text/javascript'>tableCheck();</script>\n");}if($b->selectImportPrint()){print_fieldset("import",'å¯¼å…¥',!$L);echo"<input type='file' name='csv_file'> ",html_select("separator",array("csv"=>"CSV,","csv;"=>"CSV;","tsv"=>"TSV"),$wa["format"],1);echo" <input type='submit' name='import' value='".'å¯¼å…¥'."'>","</div></fieldset>\n";}$b->selectEmailPrint(array_filter($gc,'strlen'),$f);echo"<p><input type='hidden' name='token' value='$T'></p>\n","</form>\n";}}if(is_ajax()){ob_end_clean();exit;}}elseif(isset($_GET["variables"])){$xg=isset($_GET["status"]);page_header($xg?'çŠ¶æ€':'å˜é‡');$Ih=($xg?show_status():show_variables());if(!$Ih)echo"<p class='message'>".'æ— æ•°æ®ã€‚'."\n";else{echo"<table cellspacing='0'>\n";foreach($Ih
...
as$x=>$X)json_row("sum-$x",number_format($X,0,'.',','));json_row("");}elseif($_GET["script"]=="kill")$g->query("KILL ".(+$_POST["kill"]));else{foreach(count_tables($b->databases())as$k=>$X)json_row("tables-".js_escape($k),$X);json_row("");}exit;}else{$Ng=array_merge((array)$_POST["tables"],(array)$_POST["views"]);if($Ng&&!$l&&!$_POST["search"]){$I=true;$ee="";if($w=="sql"&&count($_POST["tables"])>1&&($_POST["drop"]||$_POST["truncate"]||$_POST["copy"]))queries("SET foreign_key_checks = 0");if($_POST["truncate"]){if($_POST["tables"])$I=truncate_tables($_POST["tables"]);$ee='å·²æ¸…ç©ºè¡¨ã€‚';}elseif($_POST["move"]){$I=move_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$ee='å·²è½¬ç§»è¡¨ã€‚';}elseif($_POST["copy"]){$I=copy_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$ee='è¡¨å·²å¤åˆ¶ã€‚';}elseif($_POST["drop"]){if($_POST["views"])$I=drop_views($_POST["views"]);if($I&&$_POST["tables"])$I=drop_tables($_POST["tables"]);$ee='å·²åˆ é™¤è¡¨ã€‚';}elseif($w!="sql"){$I=($w=="sqlite"?queries("VACUUM"):apply_queries("VACUUM".($_POST["optimize"]?"":" ANALYZE"),$_POST["tables"]));$ee='Tables have been optimized.';}elseif(!$_POST["tables"])$ee='æ²¡æœ‰è¡¨ã€‚';elseif($I=queries(($_POST["optimize"]?"OPTIMIZE":($_POST["check"]?"CHECK":($_POST["repair"]?"REPAIR":"ANALYZE")))." TABLE ".implode(", ",array_map('idf_escape',$_POST["tables"])))){while($K=$I->fetch_assoc())$ee.="<b>".h($K["Table"])."</b>: ".h($K["Msg_text"])."<br>";}queries_redirect(substr(ME,0,-1),$ee,$I);}page_header(($_GET["ns"]==""?'æ•°æ®åº“'.": ".h(DB):'æ¨¡å¼'.": ".h($_GET["ns"])),$l,true);if($b->homepage()){if($_GET["ns"]!==""){echo"<h3 id='tables-views'>".'è¡¨å’Œè§†å›¾'."</h3>\n";$Mg=tables_list();if(!$Mg)echo"<p class='message'>".'æ²¡æœ‰è¡¨ã€‚'."\n";else{echo"<form action='' method='post'>\n";if(support("table")){echo"<fieldset><legend>".'åœ¨è¡¨ä¸­æœç´¢æ•°æ®'." <span id='selected2'></span></legend><div>","<input type='search' name='query' value='".h($_POST["query"])."'> <input type='submit' name='search' value='".'æœç´¢'."'>\n","</div></fieldset>\n";if($_POST["search"]&&$_POST["query"]!="")search_tables();}echo"<table cellspacing='0' class='nowrap checkable' onclick='tableClick(event);' ondblclick='tableClick(event, true);'>\n",'<thead><tr class="wrap"><td><input id="check-all" type="checkbox" onclick="formCheck(this, /^(tables|views)\[/);">','<th>'.'è¡¨','<td>'.'å¼•æ“','<td>'.'æ ¡å¯¹','<td>'.'æ•°æ®é•¿åº¦','<td>'.'ç´¢å¼•é•¿åº¦','<td>'.'æ•°æ®ç©ºé—²','<td>'.'è‡ªåŠ¨å¢é‡','<td>'.'è¡Œæ•°',(support("comment")?'<td>'.'æ³¨é‡Š':''),"</thead>\n";$S=0;foreach($Mg
as$C=>$U){$Lh=($U!==null&&!preg_match('~table~i',$U));echo'<tr'.odd().'><td>'.checkbox(($Lh?"views[]":"tables[]"),$C,in_array($C,$Ng,true),"","formUncheck('check-all');"),'<th>'.(support("table")?'<a href="'.h(ME).'table='.urlencode($C).'" title="'.'æ˜¾ç¤ºç»“æ„'.'">'.h($C).'</a>':h($C));if($Lh){echo'<td colspan="6"><a href="'.h(ME)."view=".urlencode($C).'" title="'.'ä¿®æ”¹è§†å›¾'.'">'.'è§†å›¾'.'</a>','<td align="right"><a href="'.h(ME)."select=".urlencode($C).'" title="'.'é€‰æ‹©æ•°æ®'.'">?</a>';}else{foreach(array("Engine"=>array(),"Collation"=>array(),"Data_length"=>array("create",'ä¿®æ”¹è¡¨'),"Index_length"=>array("indexes",'ä¿®æ”¹ç´¢å¼•'),"Data_free"=>array("edit",'æ–°å»ºæ•°æ®'),"Auto_increment"=>array("auto_increment=1&create",'ä¿®æ”¹è¡¨'),"Rows"=>array("select",'é€‰æ‹©æ•°æ®'),)as$x=>$_){$s=" id='$x-".h($C)."'";echo($_?"<td align='right'>".(support("table")||$x=="Rows"?"<a href='".h(ME."$_[0]=").urlencode($C)."'$s title='$_[1]'>?</a>":"<span$s>?</span>"):"<td id='$x-".h($C)."'>&nbsp;");}$S++;}echo(support("comment")?"<td id='Comment-".h($C)."'>&nbsp;":"");}echo"<tr><td>&nbsp;<th>".sprintf('å…±è®¡ %d',count($Mg)),"<td>".nbsp($w=="sql"?$g->result("SELECT @@storage_engine"):""),"<td>".nbsp(db_collation(DB,collations()));foreach(array("Data_length","Index_length","Data_free")as$x)echo"<td align='right' id='sum-$x'>&nbsp;";echo"</table>\n";if(!information_schema(DB)){$Fh="<input type='submit' value='".'Vacuum'."'".on_help("'VACUUM'")."> ";$Ie="<input type='submit' name='optimize' value='".'ä¼˜åŒ–'."'".on_help($w=="sql"?"'OPTIMIZE TABLE'":"'VACUUM OPTIMIZE'")."> ";echo"<fieldset><legend>".'Selected'." <span id='selected'></span></legend><div>".($w=="sqlite"?$Fh:($w=="pgsql"?$Fh.$Ie:($w=="sql"?"<input type='submit' value='".'åˆ†æ'."'".on_help("'ANALYZE TABLE'")."> ".$Ie."<input type='submit' name='check' value='".'æ£€æŸ¥'."'".on_help("'CHECK TABLE'")."> "."<input type='submit' name='repair' value='".'ä¿®å¤'."'".on_help("'REPAIR TABLE'")."> ":""))).(support("table")?"<input type='submit' name='truncate' value='".'æ¸…ç©º'."'".confirm().on_help($w=="sqlite"?"'DELETE'":"'TRUNCATE".($w=="pgsql"?"'":" TABLE'"))."> ":"")."<input type='submit' name='drop' value='".'åˆ é™¤'."'".confirm().on_help("'DROP TABLE'").">\n";$j=(support("scheme")?$b->schemas():$b->databases());if(count($j)!=1&&$w!="sqlite"){$k=(isset($_POST["target"])?$_POST["target"]:(support("scheme")?$_GET["ns"]:DB));echo"<p>".'è½¬ç§»åˆ°å…¶å®ƒæ•°æ®åº“'.": ",($j?html_select("target",$j,$k):'<input name="target" value="'.h($k).'" autocapitalize="off">')," <input type='submit' name='move' value='".'è½¬ç§»'."'>",(support("copy")?" <input type='submit' name='copy' value='".'å¤åˆ¶'."'>":""),"\n";}echo"<input type='hidden' name='all' value='' onclick=\"selectCount('selected', formChecked(this, /^(tables|views)\[/));".(support("table")?" selectCount('selected2', formChecked(this, /^tables\[/) || $S);":"")."\">\n";echo"<input type='hidden' name='token' value='$T'>\n","</div></fieldset>\n";}echo"</form>\n","<script type='text/javascript'>tableCheck();</script>\n";}echo'<p class="links"><a href="'.h(ME).'create=">'.'åˆ›å»ºè¡¨'."</a>\n",(support("view")?'<a href="'.h(ME).'view=">'.'åˆ›å»ºè§†å›¾'."</a>\n":"");if(support("routine")){echo"<h3 id='routines'>".'å­ç¨‹åº'."</h3>\n";$Zf=routines();if($Zf){echo"<table cellspacing='0'>\n",'<thead><tr><th>'.'åç§°'.'<td>'.'ç±»å‹'.'<td>'.'è¿”å›ç±»å‹'."<td>&nbsp;</thead>\n";odd('');foreach($Zf
